...
model = RobertaModel.from_pretrained("codebert-python")   
...
score = scores.detach().numpy().flatten().tolist()
list = sorted(enumerate(score), key=lambda x:x[1], reverse=True)  
score_positon = [x[0] for x in list] 
for i in range(3):       # Top = 3
    code_position = score_positon[i]
    print("Code:",codes[code_position])
    print("Score:",scores[0,code_position].item())   
